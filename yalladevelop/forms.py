...
class ContactForm(forms.Form):
    ...
    sender = forms.EmailField(required=True, label="Email", widget=forms.TextInput(
    ...
    subject = forms.CharField(max_length=100, required=True, widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': "What is your concern?"}))
...
class ForgotForm(forms.Form):
    email = forms.EmailField(required=False, widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': "The email you registered with"}))
...
class AddForm(forms.Form):
    project_name = forms.CharField(max_length=100, required=True, widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': "Project Name"}))
    description = forms.CharField(required=True, widget=forms.Textarea(
        attrs={'class': 'form-control', 'placeholder': "Your project description."}))
    target_money = forms.IntegerField(required=True, widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': "e.g. 10000"}))
    image = forms.ImageField(required=True,
                             help_text="Images are required. Upload an image of the concept or the wireframes of your project, and bring it to life!",
                             widget=forms.FileInput(attrs={'class': 'form-control', 'placeholder': "e.g. 10000"}))
...
class UserCreateForm(UserCreationForm):
    name = forms.CharField(max_length=200,
                           widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'FirstName LastName'}))
    email = forms.EmailField(required=True, widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': 'Please enter a valid email address so we can reach you.'}))
    ...
    username = forms.CharField(max_length=200, widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': 'Your username (Only letters and numbers, no spaces allowed)'}))
    password1 = forms.CharField(label="Password", max_length=200, widget=forms.PasswordInput(
        attrs={'class': 'form-control',
               'placeholder': 'For your security, we will never store your password as text.'}))
...
class UserUpdateForm(forms.Form):
    ...
    email = forms.EmailField(required=True, widget=forms.TextInput(attrs={'class': 'form-control',
                                                                          'placeholder': 'Please enter a valid email address so we can reach you. No spam. Ever.'}))

    password1 = forms.CharField(label="Old Password", widget=forms.PasswordInput(
        attrs={'class': 'form-control', 'placeholder': 'Enter your password to save the changes.'}), required=False)
    password2 = forms.CharField(label="New Password?", widget=forms.PasswordInput(
        attrs={'class': 'form-control', 'placeholder': 'Only enter new password if you want to change it.'}),
                                required=False)
    password3 = forms.CharField(label="Confirm New Password", widget=forms.PasswordInput(
        attrs={'class': 'form-control', 'placeholder': 'Confirm New Password'}), required=False)
    ...
    def update(self, d):
        ...
        if (self.cleaned_data['password1'] != ""):  # old password is not empty
            if u.check_password(self.cleaned_data['password1']):
                ...
                if (self.cleaned_data['password2'] != "") and (
                        self.cleaned_data['password2'] == self.cleaned_data['password3']):
...
class CompanyCreateForm(UserCreationForm):
    name = forms.CharField(max_length=200,
                           widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': "Company's Email"}))
    email = forms.EmailField(required=True,
                             widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': "Company's Email"}))
    image = forms.ImageField(required=True, widget=forms.FileInput(attrs={'class': 'form-control'}))

    username = forms.CharField(max_length=200, widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': "Company's Username"}))
    password1 = forms.CharField(label="Password", max_length=200, widget=forms.PasswordInput(
        attrs={'class': 'form-control',
               'placeholder': 'For your security, we will never store your password as text.'}))
    password2 = forms.CharField(label="Password Confirmation", max_length=200, widget=forms.PasswordInput(
    ...
    def save(self, commit=True, image=None):
        ...
        if commit and image:
            user.save()
            userProfile = UserProfile(user=user, name=fullName, is_company=True, image=image,
