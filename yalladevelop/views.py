...
def getVariables(request, dictionary={}):
    ...
    if dictionary:
        ...
        return {'user': user, 'logged_in': logged_in, 'not_logged_in': not_logged_in, 'is_company': user_is_company,
...
def addProject(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            p = Project(name=name, user_id=request.user.id, target_money=money, description=description, image=image,
...
def allprojects(request):
    ...
    return render(request, 'yalladevelop/allprojects.html', d)
...
def allcompanies(request):
    ...
    d['users'] = users
    d['paginator'] = paginator
    return render(request, 'yalladevelop/allcompanies.html', d)
...
def track(request):
    ...
    return render(request, 'yalladevelop/track.html', d)
...
def explore(request):
    ...
    if Project.objects.all():
        ...
        d['projects'] = projects
    return render(request, 'yalladevelop/explore.html', d)
...
def search_skills(request, skill_id=0):
    ...
    if skill_id == 0:
        ...
        return render(request, 'yalladevelop/search_skills.html', d)
    else:
...
def postComment(request):
    ...
    if len(comment) > 1:
        newComment = Comment(project_id=project_id, comment=comment, username=username, user_id=userId,
...
def getProfilePicture(request, user_id):
    ...
    return HttpResponse(e.image.read(), content_type=e.mimetype)


# Reading the project picture
def getProjectPicture(request, project_id):
    p = Project.objects.get(id=project_id)
    return HttpResponse(p.image.read(), content_type=p.mimetype)
